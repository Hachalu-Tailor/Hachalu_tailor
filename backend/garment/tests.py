...
class GarmentFlowTests(APITestCase):
	...
	def test_garment_endpoints_list_detail_and_process(self):
		...
		in_progress_list = self.client.get("/api/garment/orders/in-progress/?customer=Garment")
		self.assertEqual(in_progress_list.status_code, status.HTTP_200_OK)
		self.assertTrue(any(item["order_code"] == order.order_code for item in in_progress_list.data))
		...
		self.assertTrue(any(item["order_code"] == order.order_code for item in shipped_list.data))
