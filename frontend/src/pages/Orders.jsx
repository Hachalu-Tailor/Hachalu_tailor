          ...
          <div className="flex flex-col items-center justify-center py-20">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md bg-zinc-50 dark:bg-zinc-900 p-10 rounded-3xl border dark:border-white/5 shadow-2xl">
              <div className="text-center mb-10">
                ...
                <h2 className="text-3xl font-black dark:text-white uppercase tracking-tighter">{t('orders.trackOrder')}</h2>
                <p className="text-gray-500 text-[10px] uppercase tracking-[0.2em] mt-2">{t('orders.enterCode')}</p>
              </div>
              ...
              <form onSubmit={handleTrack} className="space-y-4">
                ...
                <button disabled={isSearching} className="w-full bg-black dark:bg-white text-white dark:text-black font-black uppercase py-5 rounded-2xl tracking-widest text-xs hover:bg-red-600 dark:hover:bg-red-600 dark:hover:text-white transition-all flex items-center justify-center gap-3">
                  {isSearching ? t('orders.verifying') : t('orders.accessDashboard')}
                </button>
              </form>
            </motion.div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-7 space-y-6">
              <header className="flex justify-between items-center bg-black p-8 rounded-3xl text-white">
                <div>
                  <p className="text-[10px] font-bold text-red-500 uppercase tracking-widest">{t('orders.activeOrder')}</p>
                  <h2 className="text-4xl font-black tracking-tighter">{orderCode}</h2>
                </div>
                <button onClick={() => setOrderDetails(null)} className="text-[10px] uppercase font-bold border-b border-white/20 pb-1">{t('orders.exit')}</button>
              </header>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4">
                        <span className={`text-[8px] font-black uppercase px-2 py-1 rounded-md ${pay.is_verified ? 'bg-green-500/10 text-green-500' : 'bg-orange-500/10 text-orange-500'}`}>
                          {pay.is_verified ? t('orders.verified') : t('orders.pending')}
                        </span>
                        <HiOutlineEye className="text-gray-300 group-hover:text-red-600" />
                      </div>
                    </div>
                  )) : (
                    <p className="text-center py-10 text-gray-400 text-[10px] uppercase tracking-widest">{t('orders.noPayments')}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-5 space-y-6">
              <div className="bg-white dark:bg-zinc-900 p-8 rounded-3xl border dark:border-white/5 shadow-xl">
                <h3 className="text-black dark:text-white font-black uppercase text-xs tracking-widest mb-6">{t('orders.securePayment')}</h3>
            ...
            >
              ...
              <div className="p-8 space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <DetailItem label={t('orders.transactionId')} value={selectedPayment.bank_ref_number} />
                  <DetailItem label={t('orders.amountPaid')} value={`ETB ${selectedPayment.payment_amount}`} />
                  <DetailItem label={t('orders.orderStatus')} value={selectedPayment.is_verified ? t('orders.verified') : t('orders.underReview')} highlight={selectedPayment.is_verified} />
